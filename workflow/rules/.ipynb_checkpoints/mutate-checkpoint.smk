...
rule mutate:
    ...
    """
    input:
        trees="{outdir}/{species}_{model}/{pop_str}/{contig}_{start}_{end}_sim_seed{seed}.init.trees"
    output:
        trees="{outdir}/{species}_{model}/{pop_str}/{contig}_{start}_{end}_sim_seed{seed}.mutated.trees",
        log="{outdir}/{species}_{model}/{pop_str}/{contig}_{start}_{end}_sim_seed{seed}.mutated.log"
    params:
    ...
    log:
        "logs/{outdir}/{species}_{model}/{pop_str}/{contig}_{start}_{end}_sim_seed{seed}.mutated.snakemake.log"
    script:
