...
def main():
    ...
    with open(output_ancestral, "wb") as raw:
        with BGZipWriter(raw) as f:
            seq = f">{contig_name}\n" + "".join(ancestral_states) + "\n"
            f.write(seq.encode("utf-8"))
    
    with open(output_outgroup, "wb") as raw:
        with BGZipWriter(raw) as f:
            seq = f">{contig_name}\n" + "".join(ancestral_states) + "\n"
            f.write(seq.encode("utf-8"))
    ...
    with open(output_log, "w") as f:
        f.write(f"VCF Log\n")
        f.write(f"=" * 50 + "\n")
        f.write(f"VCF written: {output_vcf}")
        ...
        f.write(f"ancestral fasta written to: {output_ancestral}\n")
        f.write(f"outgroup fasta written to: {output_outgroup}\n")
        f.write(f"Timing:\n")
